...
;; interpreter does?
;;
;; `framing_kotoba_test.clj` drives the guest through `kotoba.kir`, which is not what
  ...
  (delay
    (let [{:keys [kotoba nbb runtime]} @tools]
      (if-not (and kotoba nbb runtime)
        ...
        (let [r (shell/sh nbb "test/mime/artifact_probe.cljs" kotoba amu-root)
...
(def ^:private kir
  (delay (:kir (compiler/compile-project
                {'mime.framing (slurp (io/file (System/getProperty "user.dir") "kotoba/mime/framing.kotoba"))}
...
(deftest the-native-backend-admits-this-guest
  ...
  (if-let [bin (:kotoba @tools)]
    (let [out (str (System/getProperty "java.io.tmpdir") "/mime-framing-native.kexe")
          r (shell/sh bin "-M" "compile"
                      (str (io/file (System/getProperty "user.dir") "kotoba/mime/framing.kotoba"))
